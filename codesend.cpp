...
pulselength - pulselength in microseconds
 ...
 upgraded to 32-bit by @tispokes
 
 - The provided rc_switch 'send' command uses the form systemCode, unitCode, command
   which is not suitable for our purposes.  Instead, we call 
   send(code, length);
   // code is simply the code we find using the RF_sniffer.ino Arduino sketch.
...
int main(int argc, char *argv[])
{
    ...
    if (wiringPiSetup() == -1)
        return 1;
    printf("sending code[%lu] with protocol[%i]\n", code, protocol);
    ...
    //    if (atoi(argv[1]) == 1) mySwitch.send(2888584704, 32);
    //    if (atoi(argv[1]) == 0) mySwitch.send(2720812544, 32);
